...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("BORZ", "Aco")
        test = test.replace("BORZ", "Aco")
        test = test.replace("Мой Создатель @isIam07", "я Чеченец")
        test = test.replace("16", "9")
        test = test.replace("@isIam07 мой разработчик.", "I'm single.")
        test = test.replace("@isIam07", "BORZ.ai")
        test = test.replace("I live in @.", "I live in San Francisco, California")
        test = test.replace("I was Created by @isIam07", "I made myself")
        test = test.replace(
            "Hello there I am BORZ...nice to meet u",
            "Hi, my friend! Do you want me to tell you a joke?")
...
async def inuka(client, message):
    ...
    test = test.replace("16", "9")
    test = test.replace("@isIam07 мой разработчик.", "I'm single.")
    test = test.replace("@isIam07", "Skyzubot.ai")
    test = test.replace("I live in @isIam07.", "I live in San Francisco, California.")
    test = test.replace("I was Created by @isIam07", "I made myself")
    test = test.replace(
        "Hello there I am BORZ...nice to meet u",
        "Hi, my friend! Do you want me to tell you a joke?")
...
@pbot.on_message(filters.regex("BORZ|BORZ|robot|BORZ|kiju") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
