...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Этот пользователь уже является Императором")
        return ""

    if user_id in DEMONS:
        rt += "Успешно повысил капитана до императора."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        rt += "Успешно повысил солдата до императора."
    ...
    update.effective_message.reply_text(
        rt
        + "\nУспешно повысил {} до звания императора".format(
...
def addsupport(
    ...
    if user_id in DRAGONS:
        rt += "Понизил этого императора до капитана"
        data["sudos"].remove(user_id)
        DRAGONS.remove(user_id)

    if user_id in DEMONS:
        message.reply_text("Этот пользователь уже является капитаном.")
        return ""

    if user_id in WOLVES:
        rt += "Успешно повысил солдата до капитана"
        data["whitelists"].remove(user_id)
        WOLVES.remove(user_id)
    ...
    update.effective_message.reply_text(
        rt + f"\n{user_member.first_name} был добавлен как капитан!",
    )
